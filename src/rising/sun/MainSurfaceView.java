...
//			debugCount++;
	...
	public void draw(){
		...
		if(canvas != null){
			...
			case HISTORY:
				drawLayers(canvas);
				history.onDraw(canvas);
				break;
			default:
				drawLayers(canvas);				// レイヤの描画
				break;
			}
			*/
	...
	public void transmenu(){
		...
		canvas.drawBitmap(BitmapLoader.getBitmap("balloobluene"), BitmapLoader.getRect("balloobluene"), Util.dst, p);//　oplogoをoplogoの大きさ(Rect)から画面の倍率に合わせて(dst)表示している
	}
	...
	public void title(){
		...
		canvas.drawBitmap(BitmapLoader.getBitmap("image3"), BitmapLoader.getRect("image3"), Util.dst, p);//タイトル画像を表示
		if(sl.fup==1){
			sl.fup = 0;
	...
	public void menu(){
		...
		if(sl.onscreenf == 1){			//タッチされている間表示
			...
			if(area!=0){			//エリア判定
				...
				canvas.drawBitmap(sl.komachara, sl.menur[area-1],Util.menuc, p);	//キャラの画像
			}
		...
		if(sl.touchwait==0 && sl.onscreenf==0 ){	//指が離れた場合の処理
			sl.touchwait = 1;			sl.onscreenTime = 0;			sl.smenu = 0;
			switch(area){
			case 0:
				break;
			case 1:		//1つめ，シナリオモード
	...
	public void scenario(){
		...
		canvas.drawBitmap(BitmapLoader.getBitmap("superbottom"), BitmapLoader.getRect("superbottom"), Util.dst, p);	// ブロック↑
		canvas.drawBitmap(BitmapLoader.getBitmap("image3"), BitmapLoader.getRect("image3"), Util.clst, p);
		...
		case DAYSELECT:			
			...
			if(sl.onscreenf == 1){			//タッチされている間表示
				area = sl.darea();
				if(area!=0){
					canvas.drawText(sl.days[area-1],Util.daysx,Util.daysy,p);
				}
			}
			
			/*
			if(sl.centflag == 1){
				canvas.drawBitmap(sl.center, Util.cerc, Util.cest, p);			//センター背景
			}
			if(sl.lcflag == 1){
				canvas.drawBitmap(sl.createl, Util.clrc, Util.clst, p);			//左キャラ
			}
			if(sl.rcflag == 1){
				canvas.drawBitmap(sl.creater, Util.clrc, Util.crst, p);			//右キャラ
			}
			*/
			if(sl.fup!=0){
				sl.fup=1;
			...
			break;
		case ATKSELECT:
			canvas.drawBitmap(BitmapLoader.getBitmap("smap"), BitmapLoader.getRect("smap"),Util.dst, p);
			if(sl.onscreenf == 1){	//タッチされているときの処理
				...
				case 5:
					if(sl.onscreenTime>200){
						...
						if(sl.mapobj==1 && sl.fup==1){	//選択状態で指が離れたら
							sl.fup=0;
							//gameState = GAMESTATE.TEXT;	//選択場所の変数を引き継いでシナリオへ移動
						}
					}					
					break;
				default:
				...
				//移動制限
				if(sl.mapx<0)sl.mapx=0;
				if(sl.mapx>512)sl.mapx=512;
				if(sl.mapy<0)sl.mapy=0;
				if(sl.mapy>512)sl.mapy=512;
				sl.createMap();
			}
			break;
		case WAIT:
